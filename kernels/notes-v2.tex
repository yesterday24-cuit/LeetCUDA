...
\usepackage[paperwidth=210mm,paperheight=297mm,left=0.75cm,right=0.75cm,top=1.0cm,bottom=1.15cm]{geometry}
\usepackage{listings,xcolor,fancyhdr,xcolor}
\usepackage{graphicx}
...
\begin{center}
  ...
  \vspace{0.6cm}
  \textbf{37 个 Kernel} · \textbf{10 个 Phase} · \textbf{全中文注释}
  }
...
// Block: (H, 1, 1)，由外层 dispatch 选择 H=32/64/128/256/512/1024，一个 block
// 处理一个 token
...
__global__ void online_safe_softmax_f32_per_token_kernel(const float *x,
  ...
  // 只有 local_tid < 32 的线程参与；shared[0] 在第二次 __syncthreads 后才对整个
  // block 可见
...
// 个元素，提高计算密度
// Level 3 — Vectorize（向量化访存）：float4/half2，减少
// load/store 指令数
// Level 4 — Tensor Core（MMA
// m16n8k16）：硬件矩阵乘单元，warp 级指令
// Level 5 — Warp Specialization +
...
// - bx = blockIdx.z * gridDim.x + blockIdx.x，把连续 block 打散到不同 N 区域，改善
// L2 命中
...
__global__ void __launch_bounds__(256)
  ...
  // 加 B_PAD 来打散 bank 映射，避免按列访问时出现明显 bank conflict。这里先保留最简
  // PAD=0 版本。
...
// 这些字段是当前 WGMMA/TMA 布局下的实现常量，不要把它直接背成 BM/BK
// 的原始字节数公式。
...
__global__ void __launch_bounds__(NUM_THREADS)
  ...
  if (threadIdx.x == 0) {
    for (int i = 0; i < K_STAGE; ++i) {
      ...
      // 而是这个 barrier 上每轮会 arrive 的参与者总数：128 个 consumer 线程 + 1 个
      // producer 提交线程。
  ...
  if (wg_idx == 0) {
    if (tid == 0) { // 仅 producer 的 thread 0 执行 TMA 加载
      // TMA 是硬件 DMA 提交指令：发起一次 descriptor+坐标提交后，
      // 后续搬运由硬件异步完成，
  ...
  // ========== Consumer Warpgroup (WG1) ==========
    ...
    for (int block_k_iter = 0; block_k_iter < num_blocks_k;
      ...
      // fence 是为后续 WGMMA 建立可见性/顺序关系；commit 表示当前这组 WGMMA
      // 已经发完；
...
#pragma unroll
  ...
  // 注意：Q 由 blockIdx.x 固定到当前 Q tile；而 K/V 的 seqlen 遍历始终从 tile 0
  // 开始，
...
#pragma unroll 1
  for (int tile_K_seqlen = 0; tile_K_seqlen < Tc; ++tile_K_seqlen) {
    ...
    if constexpr (kStage > 1) {
      ...
      // smem_sel 负责“当前正在计算”的 K tile，smem_sel_next 负责“下一轮预取”的 K
      // tile。
      // 若 kStage=1，这两个槽位永远都等于 0，当前 K 还没算完就无法安全覆盖同一块
      // smem。
